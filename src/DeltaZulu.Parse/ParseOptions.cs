...
public enum ParseOptions : uint
{
    ...
    /// <summary>
    /// Maintain per-node usage counters during parsing (see
    /// <see cref="ParseContext.GetStats"/>). Off by default: the counters
    /// write to memory shared by all concurrent Parse calls, which costs
