...
/// more rulebases, then call <see cref="Parse(string, out JsonObject)"/> for each message.
/// </para>
/// <para>
/// Parsing runs against an immutable compiled snapshot of the rulebase,
/// so concurrent <see cref="Parse(string, out JsonObject)"/> calls are always safe. Rulebases may
/// be loaded at any time, including after parsing has started ("hot
/// reload"): a load invalidates the snapshot and the next Parse compiles
/// and atomically publishes a new one, while in-flight parses finish
...
public sealed class ParseContext
{
    ...
    /// <summary>
    /// Total node-visit and backtrack counts accumulated by parsing.
    /// Requires <see cref="ParseOptions.CollectStats"/> (set before first
    ...
    /// at any time, including after parsing has started.
    /// </summary>
    ...
    /// </para>
    /// </summary>
    public int Parse(string message, out JsonObject result)
        => PdagWalker.Parse(this, EnsureCompiled(), message, out result);

    /// <summary>
    /// Parse a message into a flat <see cref="ParseResult"/> whose
    ...
    /// </summary>
    public int Parse(string message, out ParseResult result)
    {
        var r = PdagWalker.Parse(this, EnsureCompiled(), message, out FieldCollector fields);
        result = new ParseResult(r, fields);
        return r;
    }
    ...
    /// </summary>
    public int ParseToString(string message, out string json)
    {
        var r = Parse(message, out ParseResult result);
