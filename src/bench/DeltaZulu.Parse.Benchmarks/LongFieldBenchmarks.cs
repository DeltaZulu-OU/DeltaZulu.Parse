...
public class LongFieldBenchmarks
{
    ...
    [Benchmark]
    public int LongCharTo() => _ctx.Parse(_charToMsg, out JsonObject _);

    [Benchmark]
    public int LongCharToFlat() => _ctx.Parse(_charToMsg, out ParseResult _);

    [Benchmark]
    public int LongLiteral() => _ctx.Parse(_literalMsg, out JsonObject _);

    [Benchmark]
    public int LongQuoted() => _ctx.Parse(_quotedMsg, out JsonObject _);

    [Benchmark]
    public int LongWord() => _ctx.Parse(_wordMsg, out JsonObject _);

    [Benchmark]
    public int LongWordFlat() => _ctx.Parse(_wordMsg, out ParseResult _);

    [GlobalSetup]
    public void Setup()
    {
        _ctx = new ParseContext {
        ...
        {
            if (_ctx.Parse(msg, out JsonObject _) != 0)
            {
