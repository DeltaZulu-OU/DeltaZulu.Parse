...
internal static class DateTimeParsers
{
        ...
        int hour, int minute, int second, int offsetHour, int offsetMinute, char offsetMode)
    {
        ...
        long t = days * 86400 + hour * 3600L + minute * 60L + second;
        int utcOffset = offsetHour * 3600 + offsetMinute * 60;
        if (offsetMode == '+')
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        string s = npb.Str;
        int i = offs;
        int len = npb.StrLen - offs;
        int orglen = len;

        int year = HParseInt(s, ref i, ref len);

        /* we accept slightly malformed timestamps, e.g. 2003-9-1T1:0:0 */
        if (len == 0 || s[i++] != '-') return ErrorCodes.WrongParser;
        --len;
        int month = HParseInt(s, ref i, ref len);
        if (month < 1 || month > 12) return ErrorCodes.WrongParser;

        if (len == 0 || s[i++] != '-') return ErrorCodes.WrongParser;
        --len;
        int day = HParseInt(s, ref i, ref len);
        if (day < 1 || day > 31) return ErrorCodes.WrongParser;

        if (len == 0 || s[i++] != 'T') return ErrorCodes.WrongParser;
        --len;

        int hour = HParseInt(s, ref i, ref len);
        if (hour < 0 || hour > 23) return ErrorCodes.WrongParser;

        if (len == 0 || s[i++] != ':') return ErrorCodes.WrongParser;
        --len;
        int minute = HParseInt(s, ref i, ref len);
        if (minute < 0 || minute > 59) return ErrorCodes.WrongParser;

        if (len == 0 || s[i++] != ':') return ErrorCodes.WrongParser;
        --len;
        int second = HParseInt(s, ref i, ref len);
        if (second < 0 || second > 60) return ErrorCodes.WrongParser;
        ...
        if (len == 0) return ErrorCodes.WrongParser;
        int offsetHour;
        int offsetMinute;
        char offsetMode;
        if (s[i] == 'Z')
        ...
        else if (s[i] == '+' || s[i] == '-')
        {
            ...
            if (offsetHour < 0 || offsetHour > 23) return ErrorCodes.WrongParser;
            if (len == 0 || s[i++] != ':') return ErrorCodes.WrongParser;
            --len;
            offsetMinute = HParseInt(s, ref i, ref len);
            if (offsetMinute < 0 || offsetMinute > 59) return ErrorCodes.WrongParser;
        }
        else
        {
        ...
        if (wantValue)
        {
            ...
            {
                long timestamp = SyslogTimeToUnix(year, month, day, hour, minute, second,
                ...
                if (data.FmtMode == FormatMode.AsTimestampUnixMs)
                {
                    ...
                    if (secfracPrecision == 1)
                        ...
                        for (int k = 0; k < secfracPrecision - 3; ++k)
                            div *= 10;
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        if (len < 3)
            return ErrorCodes.WrongParser;
        int month = (char.ToUpperInvariant(s[i]), char.ToUpperInvariant(s[i + 1]), char.ToUpperInvariant(s[i + 2])) switch
        {
            ('J', 'A', 'N') => 1,
            ('F', 'E', 'B') => 2,
            ('M', 'A', 'R') => 3,
        ...
        if (wantValue)
        {
            ...
            else
            {
                ...
                long timestamp = SyslogTimeToUnix(year, month, day, hour, minute, second, 0, 0, '+');
                if (data.FmtMode == FormatMode.AsTimestampUnixMs)
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        i += 6;
        for (int j = 0; j < 7 && i < npb.StrLen && TextRules.IsDigit(s[i]); ++j)
            ++i; /* just scan */
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        if (!TextRules.IsDigit(s[i]) || !TextRules.IsDigit(s[i + 1])
            ...
            return ErrorCodes.WrongParser;
        if (s[i + 4] != '-') return ErrorCodes.WrongParser;
        /* month */
        if (s[i + 5] == '0')
        {
            if (s[i + 6] < '1' || s[i + 6] > '9') return ErrorCodes.WrongParser;
        }
        else if (s[i + 5] == '1')
        {
            if (s[i + 6] < '0' || s[i + 6] > '2') return ErrorCodes.WrongParser;
        }
        else
        {
            return ErrorCodes.WrongParser;
        }
        if (s[i + 7] != '-') return ErrorCodes.WrongParser;
        /* day */
        if (s[i + 8] == '0')
        {
            if (s[i + 9] < '1' || s[i + 9] > '9') return ErrorCodes.WrongParser;
        }
        else if (s[i + 8] == '1' || s[i + 8] == '2')
        {
            if (!TextRules.IsDigit(s[i + 9])) return ErrorCodes.WrongParser;
        }
        else if (s[i + 8] == '3')
        {
            if (s[i + 9] != '0' && s[i + 9] != '1') return ErrorCodes.WrongParser;
        }
        else
        {
            return ErrorCodes.WrongParser;
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        if (s[i] < '0' || s[i] > '5') return ErrorCodes.WrongParser;
        if (!TextRules.IsDigit(s[i + 1])) return ErrorCodes.WrongParser;
        if (s[i + 2] != ':') return ErrorCodes.WrongParser;
        if (s[i + 3] < '0' || s[i + 3] > '5') return ErrorCodes.WrongParser;
        if (!TextRules.IsDigit(s[i + 4])) return ErrorCodes.WrongParser;
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        {
            if (s[i + 1] < '0' || s[i + 1] > '3') return ErrorCodes.WrongParser;
        }
        else
        {
            return ErrorCodes.WrongParser;
        }
        if (!CheckMmSs(s, i)) return ErrorCodes.WrongParser;
        ...
        out int parsed, bool wantValue, ref JsonNode? value)
    {
        ...
        {
            if (s[i + 1] < '0' || s[i + 1] > '2') return ErrorCodes.WrongParser;
        }
        else
        {
            return ErrorCodes.WrongParser;
