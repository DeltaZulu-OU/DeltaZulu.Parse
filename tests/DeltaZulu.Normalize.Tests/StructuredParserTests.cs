...
public class StructuredParserTests
{
    ...
    {
        var (r, j) = Normalize("rule=:%field:json%", """{"f1": "1", "f2": 2}""");
        Assert.AreEqual(0, r);
    ...
    public void Json_ConsumesTrailingWhitespaceAsPartOfMatch()
    {
        ...
        AssertJsonEquals("""{ "field": { "f1": "1", "f2": 2 } }""",
            Normalize(rb, """{"f1": "1", "f2": 2}      """).Json);
        AssertJsonEquals("""{ "field": { "f1": "1", "f2": 2 } }""",
            Normalize(rb, """begin {"f1": "1", "f2": 2} end""").Json);
    }

    [TestMethod]
    public void Json_TwoFieldsInOneRule()
    {
        var (r, j) = Normalize("rule=:%field1:json%-%field2:json%", """{"f1": "1"}-{"f2": 2}""");
        Assert.AreEqual(0, r);
    ...
    {
        var (r, j) = Normalize("rule=:%f:checkpoint-lea%", "tcp_flags: RST-ACK; src: 192.168.0.1;");
        Assert.AreEqual(0, r);
    ...
    public void NameValueList_ParsesMultiplePairs()
    {
        ...
        AssertJsonEquals("""{ "f": { "name": "value" } }""", Normalize(rb, "name=value").Json);
        AssertJsonEquals(
            """{ "f": { "name1": "value1", "name2": "value2", "name3": "value3" } }""",
            Normalize(rb, "name1=value1 name2=value2 name3=value3").Json);
        AssertJsonEquals(
            """{ "f": { "name1": "", "name2": "value2", "name3": "value3" } }""",
            Normalize(rb, "name1= name2=value2 name3=value3 ").Json);
    }
    ...
    {
        var (r, _) = Normalize("rule=:%f:name-value-list%", message);
        Assert.AreNotEqual(0, r);
    }
}
