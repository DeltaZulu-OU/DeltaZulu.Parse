...
/// <summary>Cases exercising the <see cref="ParseContext"/> loading API directly (not single rulebase text).</summary>
[TestClass]
public class ParseContextTests
{
    ...
    [TestMethod]
    public void HotReload_ConcurrentWithParsing_IsSafe()
    {
        ...
        var readers = Enumerable.Range(0, 4).Select(_ => Task.Run(() => {
            while (!stop.IsCancellationRequested)
            {
                ...
                if (ctx.Parse("msg 0 payload", out JsonObject j) != 0
        ...
        Assert.AreEqual(0, ctx.Parse("msg 50 done", out JsonObject last));
    ...
    [TestMethod]
    public void LoadSamplesFromString_AfterParse_HotReloads()
    {
        ...
        Assert.AreEqual(0, ctx.Parse("abc hello", out JsonObject j1));
        Assert.AreEqual("hello", j1["f"]!.GetValue<string>());

        Assert.AreEqual(0, ctx.LoadSamplesFromString("rule=:abd %f:rest%"));

        Assert.AreEqual(0, ctx.Parse("abd world", out JsonObject j2));
        Assert.AreEqual("world", j2["f"]!.GetValue<string>());
        Assert.AreEqual(0, ctx.Parse("abc again", out JsonObject j3));
