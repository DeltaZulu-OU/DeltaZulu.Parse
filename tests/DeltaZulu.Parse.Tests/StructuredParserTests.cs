...
public class StructuredParserTests
{
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%f:cef%",
    ...
    public void Cef_RejectsDanglingBackslashAtEndOfLastExtensionValue()
    {
        ...
        var (r, _) = TestHelpers.Parse("rule=:%f:cef%",
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%f:checkpoint-lea%", "tcp_flags: RST-ACK; src: 192.168.0.1;");
        Assert.AreEqual(0, r);
    ...
    public void Json_ConsumesTrailingWhitespaceAsPartOfMatch()
    {
        ...
        AssertJsonEquals("""{ "field": { "f1": "1", "f2": 2 } }""",
            TestHelpers.Parse(rb, """{"f1": "1", "f2": 2}      """).Json);
        AssertJsonEquals("""{ "field": { "f1": "1", "f2": 2 } }""",
            TestHelpers.Parse(rb, """begin {"f1": "1", "f2": 2} end""").Json);
    }

    [TestMethod]
    public void Json_ExtractsEmbeddedObject()
    {
        var (r, j) = TestHelpers.Parse("rule=:%field:json%", """{"f1": "1", "f2": 2}""");
        Assert.AreEqual(0, r);
    ...
    {
        var (r1, _) = TestHelpers.Parse("rule=:%field:json%", """{"f1": "1", f2: 2}""");
        Assert.AreNotEqual(0, r1);

        /* regression guard for json-c issue #181: a bare "15:00" must not be misdetected as JSON */
        var (r2, j2) = TestHelpers.Parse("rule=:%field:json%", "15:00");
        Assert.AreNotEqual(0, r2);
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%field1:json%-%field2:json%", """{"f1": "1"}-{"f2": 2}""");
        Assert.AreEqual(0, r);
    ...
    public void NameValueList_ParsesMultiplePairs()
    {
        ...
        AssertJsonEquals("""{ "f": { "name": "value" } }""", TestHelpers.Parse(rb, "name=value").Json);
        AssertJsonEquals(
            """{ "f": { "name1": "value1", "name2": "value2", "name3": "value3" } }""",
            TestHelpers.Parse(rb, "name1=value1 name2=value2 name3=value3").Json);
        AssertJsonEquals(
            """{ "f": { "name1": "", "name2": "value2", "name3": "value3" } }""",
            TestHelpers.Parse(rb, "name1= name2=value2 name3=value3 ").Json);
    }
    ...
    {
        var (r, _) = TestHelpers.Parse("rule=:%f:name-value-list%", message);
        Assert.AreNotEqual(0, r);
    }
}
