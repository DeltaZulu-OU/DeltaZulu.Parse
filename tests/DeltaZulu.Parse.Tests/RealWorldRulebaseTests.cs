...
/// complex patterns found in actual log parsing use cases.
/// </para>
/// <para>
...
public class RealWorldRulebaseTests
{
    ...
    public void AlternativeFieldMatching()
    {
        ...
        var (r1, j1) = TestHelpers.Parse(rb, "User jsmith logged in from 192.168.1.100");
        Assert.AreEqual(0, r1);
        AssertJsonContains(j1, "src", "192.168.1.100");

        var (r2, j2) = TestHelpers.Parse(rb, "User jsmith logged in from vpn.example.com");
    ...
    public void CharSeparatedWithMultipleOccurrences()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Values: val1, val2, val3");
        Assert.AreEqual(0, r);
    ...
    public void ComplexFieldSequenceWithMixedMotifs()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "14:23:45 myhost sshd: Connection closed by 10.0.0.1");
        Assert.AreEqual(0, r);
    ...
    public void DateRfc3164WithSyslogFormat()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Oct 29 09:47:08 myhost sshd[1234]: Connection accepted");
        Assert.AreEqual(0, r);
    ...
    public void EmptyFieldInSequence()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "value1value2 value3");
        Assert.AreEqual(0, r);
    ...
    public void EscapedColonsInLiterals()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "error: PAM: Authentication failure for jsmith");
        Assert.AreEqual(0, r);
    ...
    public void EscapedDoublePercentInLiteral()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Progress: 100% complete at noon");
        Assert.AreEqual(0, r);
    ...
    public void HexEncodedBackslashInDomainUsername()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "User: MYDOMAIN\\jsmith");
        Assert.AreEqual(0, r);
    ...
    public void HexEncodedDelimiterColon()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "value1: value2");
        Assert.AreEqual(0, r);
    ...
    public void HexEncodedDelimiterSingleQuote()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "myvalue'");
        Assert.AreEqual(0, r);
    ...
    public void IgnoredFieldWithRest()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "jsmith some ignored data here");
        Assert.AreEqual(0, r);
        AssertJsonEquals("""{"user": "jsmith"}""", j);
    }
    ...
    public void IgnoredFieldWithWord()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "skipped user1 login");
        Assert.AreEqual(0, r);
    ...
    public void IPv4InIPv6Notation()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Client Address: ::ffff:192.168.1.1");
        Assert.AreEqual(0, r);
    ...
    public void MultipleRuleVariantsWithWhitespace()
    {
        ...
        var (r1, j1) = TestHelpers.Parse(rb, "SSH from 10.0.0.1 on port 22");
        Assert.AreEqual(0, r1);
        AssertJsonEquals("""{"src": "10.0.0.1", "port": "22"}""", j1);

        // Double space variant
        var (r2, j2) = TestHelpers.Parse(rb, "SSH from 10.0.0.1  on port 22");
        Assert.AreEqual(0, r2);
    ...
    public void NumberFieldWithFormatModifier()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "42");
        Assert.AreEqual(0, r);
    ...
    public void OverlappingRulePrefix()
    {
        ...
        var (r1, j1) = TestHelpers.Parse(rb, "ERR:404");
        Assert.AreEqual(0, r1);
        AssertJsonContains(j1, "code", "404");

        var (r2, j2) = TestHelpers.Parse(rb, "ERR:500 database");
        Assert.AreEqual(0, r2);
    ...
    public void PrefixDirectiveAccumulation()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "PREFIX1_PREFIX2_value");
        Assert.AreEqual(0, r);
    ...
    public void QuotedStringVariants()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Message: \"error occurred\"");
        Assert.AreEqual(0, r);
    ...
    public void StringToMotifWithCustomDelimiter()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "From user1@example.com to admin");
        Assert.AreEqual(0, r);
    ...
    public void UserDefinedTypeWithPrefix()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Service sshd listening on 192.168.1.1#22");
        Assert.AreEqual(0, r);
