...
public class BasicParserTests
{
    ...
    public void CharTo_CoercesNonStringExtradataInsteadOfThrowing()
    {
        ...
        var (r, j) = TestHelpers.Parse("""rule=:%f:char-to{"extradata":5}%5""", "12345");
        Assert.AreEqual(0, r);
    ...
    public void Duration_AcceptsOneOrTwoDigitHours(string message, string expected)
    {
        const string rb = """
            ...
            """;
        var (r, j) = TestHelpers.Parse(rb, message);
        Assert.AreEqual(0, r);
    ...
    public void Duration_RejectsOutOfRangeMinutes()
    {
        const string rb = """
            ...
            rule=:duration %field:duration%
            """;
        var (r, j) = TestHelpers.Parse(rb, "duration 37:60:42 bytes");
    ...
    {
        var (r, j) = TestHelpers.Parse($"rule=:{pattern}", message);
        Assert.AreEqual(0, r);
    ...
    public void HexNumber_RequiresWhitespaceTerminator()
    {
        ...
        var (r1, j1) = TestHelpers.Parse(rb, "here is a number 0x1234 in hex form");
        Assert.AreEqual(0, r1);
        AssertJsonEquals("""{"num": "0x1234"}""", j1);

        var (r2, j2) = TestHelpers.Parse(rb, "here is a number 0x1234in hex form");
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%f:ipv6%", addr);
        Assert.AreEqual(0, r);
    ...
    {
        var (r, _) = TestHelpers.Parse("rule=:%f:ipv6%", addr);
        Assert.AreNotEqual(0, r);
    }
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%f:ipv6%", "ABCD:EF01:2345:6789:ABCD:EF01:2345:1:6798");
        Assert.AreNotEqual(0, r);
        AssertJsonEquals("""
    ...
    public void Literal_PreservesHexEscapesAsRawByteChars()
    {
         ...
         * complete UTF-8. */
        var (r, j) = TestHelpers.Parse("""rule=:caf\xC3\xA9 %f:rest%""", "caf\u00C3\u00A9 bar");
        Assert.AreEqual(0, r);
        AssertJsonEquals("""{ "f": "bar" }""", j);

        Assert.AreNotEqual(0, TestHelpers.Parse("""rule=:caf\xC3\xA9 %f:rest%""", "café bar").Result);
    }
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:%field:mac48%", mac);
        Assert.AreEqual(0, r);
    ...
    {
        var (r, _) = TestHelpers.Parse("rule=:%field:mac48%", mac);
        Assert.AreNotEqual(0, r);
    }
    ...
    public void Number_ExtractsDigitsAndFailsOnTrailingLetters()
    {
        ...
        var (r1, j1) = TestHelpers.Parse(rb, "here is a number 1234 in dec form");
        Assert.AreEqual(0, r1);
        AssertJsonEquals("""{"num": "1234"}""", j1);

        var (r2, j2) = TestHelpers.Parse(rb, "here is a number 1234in dec form");
        Assert.AreNotEqual(0, r2);
        AssertJsonEquals("""
    ...
    public void QuotedString_StripsOuterQuotes()
    {
        ...
        AssertJsonEquals("""{ "f": "alpha beta" }""", TestHelpers.Parse(rb, "\"alpha beta\"").Json);
        AssertJsonEquals("""{ "f": "" }""", TestHelpers.Parse(rb, "\"\"").Json);

        var (r, j) = TestHelpers.Parse(rb, "\"unterminated");
        Assert.AreNotEqual(0, r);
        AssertJsonEquals("""
    ...
    public void StringParser_DashIsEmptyAcceptsPythonBooleanAndRejectsQuotedDash()
    {
        ...
        AssertJsonEquals("""{ "str": "" }""", TestHelpers.Parse(rb, "-").Json);

        var quotedDash = TestHelpers.Parse(rb, "\"-\"");
    ...
    public void FieldStringDashIsEmptyFixture_RejectsQuotedDashCases()
    {
        ...
        try
        {
            ...
            var result = ctx.Parse("\"-\"", out JsonObject json);
    ...
    public void Include_ResolvesRelativeToIncludingRulebaseFile()
    {
        ...
        try
        {
            ...
            Assert.AreEqual(0, ctx.Parse("f0:f6:1c:5f:cc:a2", out JsonObject json));
    ...
    public void Include_UsesLegacyLiblognormRulebasesEnvironmentFallback()
    {
        var dir = Path.Combine(Path.GetTempPath(), "dzn-rb-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(dir);
        var oldNew = Environment.GetEnvironmentVariable("DeltaZulu.Parse_RULEBASES");
        var oldLegacy = Environment.GetEnvironmentVariable("LIBLOGNORM_RULEBASES");
        try
        {
            File.WriteAllText(Path.Combine(dir, "inc.rulebase"), "version=2\nrule=:%mac:mac48%\n");
            Environment.SetEnvironmentVariable("DeltaZulu.Parse_RULEBASES", null);
        ...
        {
            Environment.SetEnvironmentVariable("DeltaZulu.Parse_RULEBASES", oldNew);
