...
internal static class TestHelpers
{
    public static void AssertJsonContains(JsonNode? json, string fieldName, string expectedValue)
    {
        ...
        if (trimmed.StartsWith('{') || trimmed.StartsWith('['))
        {
            ...
            if (!JsonNode.DeepEquals(SortKeys(expectedNode), SortKeys(fieldValue?.DeepClone())))
            {
    ...
    /// <summary>Assert that the parsed JSON output equals the expected JSON (order-independent).</summary>
    public static void AssertJsonEquals(string expectedJson, JsonNode? actual)
    {
        var expected = JsonNode.Parse(expectedJson);
        var equal = JsonNode.DeepEquals(SortKeys(expected), SortKeys(actual));
        if (!equal)
        {
    ...
    /// <summary>Build a context from rulebase body text (without the "version=2" header) and parse one message.</summary>
    public static (int Result, JsonObject Json) Parse(string rulebaseBody, string message, ParseOptions options = ParseOptions.None)
    {
            ...
            case JsonArray arr:
                {
                    ...
                    {
                        result.Add(SortKeys(item?.DeepClone()));
                    }
