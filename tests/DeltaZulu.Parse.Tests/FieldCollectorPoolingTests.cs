...
/// <summary>
/// The classic Parse(out JsonObject) overload rents its scratch
...
public class FieldCollectorPoolingTests
{
    ...
    [TestMethod]
    public void ConcurrentParseCalls_DoNotCorruptEachOthersResults()
    {
        ...
        var tasks = Enumerable.Range(0, Environment.ProcessorCount * 2).Select(t => Task.Run(() => {
            for (var i = 0; i < 500; i++)
            {
                ...
                if (ctx.Parse(msg, out JsonObject j) != 0
        ...
        Assert.IsTrue(Task.WaitAll(tasks, TimeSpan.FromSeconds(30)), "concurrent parsing workers did not finish within the timeout");
    ...
    public void FailedMatch_AlsoRoundTripsThePooledArrayCorrectly()
    {
        ...
        {
            var msg = $"goodbye {i}";
            Assert.AreNotEqual(0, ctx.Parse(msg, out JsonObject j));
    ...
    public void MoreFieldsThanPooledCapacity_GrowsAndReturnsCorrectResult()
    {
        ...
        Assert.AreEqual(0, ctx.Parse(msg, out JsonObject j));
