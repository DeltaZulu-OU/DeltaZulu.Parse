...
/// combined PDAG: <see cref="ParseContext.LoadSamplesFromDirectory"/>, the
/// directory auto-detection in <see cref="ParseContext.LoadSamples"/>, and
...
public class DirectoryRulebaseTests
{
    ...
    public void NonRecursiveLoadIgnoresSubdirectories()
    {
        ...
        Assert.AreNotEqual(0, ctx.Parse("inner y", out JsonObject _), "subdirectory file was loaded despite recursive: false");
    }
    ...
    public void PrefixDoesNotLeakBetweenFiles()
    {
        ...
        Assert.AreNotEqual(0, ctx.Parse("PFX two y", out JsonObject _), "prefix leaked into the second file");
    }
    ...
    private static void AssertMatches(ParseContext ctx, string message, string field, string expected)
    {
        var r = ctx.Parse(message, out JsonObject json);
        Assert.AreEqual(0, r, $"message did not parse: '{message}' -> {json.ToJsonString()}");
        TestHelpers.AssertJsonContains(json, field, expected);
    }

    private static ParseContext NewContext(List<string> errors)
