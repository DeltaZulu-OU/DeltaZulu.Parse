...
using static DeltaZulu.Parse.Tests.TestHelpers;
...
public class PdagBehaviorTests
{
    ...
    {
        var (r, j) = TestHelpers.Parse("rule=:hello %name:word%", "hello world", ParseOptions.AddRule);
        Assert.AreEqual(0, r);
    ...
    public void Alternative_TriesEachBranchInOrder()
    {
        const string rb = """
            ...
            """;
        AssertJsonEquals("""{ "num": "4711" }""", TestHelpers.Parse(rb, "a 4711 b").Json);
        AssertJsonEquals("""{ "hex": "0x4711" }""", TestHelpers.Parse(rb, "a 0x4711 b").Json);
    }
    ...
    public void Alternative_AcceptsNestedSequenceWithTrailingCommas()
    {
        const string rb = """
            ...
            """;
        AssertJsonEquals("""{ "num1": "47", "num": "11" }""", TestHelpers.Parse(rb, "a 47:11 b").Json);
        AssertJsonEquals("""{ "hex": "0x4711" }""", TestHelpers.Parse(rb, "a 0x4711 b").Json);
    }
    ...
    public void Annotate_AddsStaticFieldsPerTag()
    {
        ...
        AssertJsonEquals("""{ "tag": "TAG", "event.tags": ["WIN"], "annot1": "WIN" }""",
            TestHelpers.Parse(rb, "<37>1 2016-11-03T23:59:59+03:00 server.example.net TAG . - -").Json);
        AssertJsonEquals("""{ "tag": "TAG", "event.tags": ["ABC"], "annot2": "ABC" }""",
            TestHelpers.Parse(rb, "<37>1 2016-11-03T23:59:59+03:00 server.example.net TAG + - -").Json);
        AssertJsonEquals("""{ "tag": "TAG", "event.tags": ["ABC", "WIN"], "annot1": "WIN", "annot2": "ABC" }""",
            TestHelpers.Parse(rb, "<6>1 2016-09-02T07:41:07+02:00 server.example.net TAG - - -").Json);
    }
    ...
    public void Backtracking_PicksMostSpecificMatchingRuleAmongSharedPrefix()
    {
        ...
        AssertJsonEquals(
            ...
            TestHelpers.Parse(rb, "Outside:10.20.30.40/35 (40.30.20.10/35)").Json);

        AssertJsonEquals(
            """{ "tail": " with rest", "label2": "40.30.20.10/35", "port": "35", "ip": "10.20.30.40", "iface": "Outside" }""",
            TestHelpers.Parse(rb, "Outside:10.20.30.40/35 (40.30.20.10/35) with rest").Json);

        AssertJsonEquals(
            """{ "tail": " (40.30.20.10/35 brace missing", "port": "35", "ip": "10.20.30.40", "iface": "Outside" }""",
            TestHelpers.Parse(rb, "Outside:10.20.30.40/35 (40.30.20.10/35 brace missing").Json);

        var (r, j) = TestHelpers.Parse(rb, "Outside:10.20.30.40/aa 40.30.20.10/35");
    ...
    public void CustomType_DotDotNamePushesValueUpToCallSite()
    {
        const string rb = """
            ...
            """;
        AssertJsonEquals("""{ "ip": "10.0.0.1" }""", TestHelpers.Parse(rb, "an ip address 10.0.0.1").Json);
        AssertJsonEquals("""{ "ip": "127::1" }""", TestHelpers.Parse(rb, "an ip address 127::1").Json);
    }
    ...
    public void CustomType_NestedDotDotUnwrapsUnderNamedIntermediateType()
    {
        ...
        AssertJsonEquals("""{ "f1":"0xff", "f2":"0x16" }""",
            TestHelpers.Parse(rb, "two bytes 0xff 0x16 stop").Json);
    }
    ...
    public void CustomType_NamedFieldInsideTypeDefinition()
    {
        ...
        AssertJsonEquals("""{ "ip": "2001:DB8:0:1::10:1FF" }""",
            TestHelpers.Parse(rb, "an ip address 2001:DB8:0:1::10:1FF").Json);
    }
    ...
    public void FullJsonFieldFollowedByEscapedPercentKeepsRuleOnOneLogicalLine()
    {
        ...
        AssertJsonEquals("""{ ".": "XY7azl704-84a39894783423467a33f5b48bccd23c-a0n63i2", "resta": " LWL" }""",
            TestHelpers.Parse(rb, "Rule-ID: XY7azl704-84a39894783423467a33f5b48bccd23c-a0n63i2 LWL").Json);
    }
    ...
    public void CompiledPdagBinary_RoundTripsSnapshotForPersistence()
    {
        ...
        buffer.Position = 0;
        var restored = new ParseContext();
        restored.ImportCompiledPdag(buffer);

        Assert.AreEqual(0, restored.Parse("hello abc 123:0xff keep this!", out JsonObject roundTripped));
        AssertJsonEquals("""{ "name":"abc", "left":123, "right":"0xff", "tail":"keep this" }""", roundTripped);

        Assert.AreEqual(0, restored.Parse("json {\"a\":1}", out JsonObject jsonRoundTripped));
    ...
    public void Prefix_LineIsPrependedToEveryRule()
    {
        ...
        var (r, j) = TestHelpers.Parse(rb, "Aug 18 13:18:45 myhost hello world");
        Assert.AreEqual(0, r);
        AssertJsonEquals("""
    ...
    public void PrefixRules_ShorterRuleStillMatchesWhenLongerOneFails()
    {
        const string rb = """
            ...
            """;
        AssertJsonEquals("""{ "w2": "w2", "w1": "w1" }""", TestHelpers.Parse(rb, "a word w1 another word w2").Json);
        AssertJsonEquals("""{ "w1": "w1" }""", TestHelpers.Parse(rb, "a word w1").Json);
    }
    ...
    public void Repeat_AcceptsNestedAlternativeWithTrailingCommas()
    {
        ...
        AssertJsonEquals("""{ "numbers":[ { "n1":"1", "n2":"2" }, { "n1":"3", "n2":"4" } ] }""",
            TestHelpers.Parse(rb, "a 1:2, 3:4 b").Json);
        AssertJsonEquals("""{ "numbers":[ { "hex":"0x4711" } ] }""",
            TestHelpers.Parse(rb, "a 0x4711 b").Json);
    }
    ...
    public void Repeat_CollectsMultiFieldPerIteration()
    {
        const string rb = """
            ...
            """;
        var (r, j) = TestHelpers.Parse(rb, "a 1:2, 3:4, 5:6, 7:8 b test");
        Assert.AreEqual(0, r);
        AssertJsonEquals("""
    ...
    public void Repeat_CollectsSingleFieldPerIteration()
    {
        const string rb = """
            ...
            """;
        var (r, j) = TestHelpers.Parse(rb, "a 1, 2, 3, 4 b test");
        Assert.AreEqual(0, r);
        AssertJsonEquals("""
    ...
    public void Repeat_FailOnDuplicate_SkipsAlreadyExtractedFields()
    {
        ...
        const string rb = """
            rule=:l %{"name":".", "type":"repeat", "option.failOnDuplicate":true,
                ...
                "while": {"type":"literal", "text":" "} }%
            """;
        var (r, j) = TestHelpers.Parse(rb, "l 1 2");
        Assert.AreEqual(0, r);
    ...
    public void Repeat_PermitMismatchInParser_StopsAtLastGoodMatch()
    {
        const string rb = """
            ...
            """;
        var (r, j) = TestHelpers.Parse(rb, "l 1 2 END");
        Assert.AreEqual(0, r);
    ...
    public void Repeat_StopsWhenParserAndWhileBothMatchZeroWidth()
    {
        ...
        const string rb = """
            ...
            """;
        var task = System.Threading.Tasks.Task.Run(() => TestHelpers.Parse(rb, "a ,b"));
